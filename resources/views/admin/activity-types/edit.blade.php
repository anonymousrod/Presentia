...
<div class="container-fluid py-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h2 class="mb-0">Modifier: {{ $activityType->name }}</h2>
        ...
        <div class="card-body p-4">
            <form action="{{ route('admin.activity-types.update', $activityType) }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control @error('name') is-invalid @enderror" id="name" name="name" value="{{ old('name', $activityType->name) }}" required>
                    @error('name')
                ...
                <div class="mb-4">
                    ...
                    <input type="color" class="form-control form-control-color @error('color') is-invalid @enderror" id="color" name="color" value="{{ old('color', $activityType->color) }}" title="Choisir une couleur" required>
                    @error('color')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
